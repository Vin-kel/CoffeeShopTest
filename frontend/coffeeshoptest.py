...
st.set_page_config(
    page_title="Kasir & Analytics POS",
    layout="wide",
...
if menus_data is None:
    st.error("⚠️ Gagal terhubung ke Server FastAPI. Pastikan server backend Anda sudah dijalankan (`uvicorn main:app --reload`).")
    st.stop()


st.sidebar.title("🏪 POS System")
st.sidebar.markdown("---")
menu_options = ["📊 Dashboard Analytics", "🛍️ Transaksi Baru", "🍔 Manajemen Menu"]
...
elif choice == "🛍️ Transaksi Baru":
    st.title("🛍️ Transaksi Kasir Baru")
